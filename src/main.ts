...
function animateOnce(el: Element, cls: string) {
  el.classList.add(cls);
  const off = () => (el as HTMLElement).classList.remove(cls);
...
addNoteBtn.addEventListener("click", async () => {
  ...
  if (listWrap) animateOnce(listWrap, "flash-list");

  // uh.. animations!
  const first = notesList?.querySelector(".note-item");
  if (first) animateOnce(first, "pop-in-strong");
  if (notesList) animateOnce(notesList.parentElement!, "flash-list");
...
clearAllBtn.addEventListener("click", async () => {
  ...
  await saveNotes([]);
  renderNotes([]);
  if (notesList) animateOnce(notesList, "flash-list");
});
...
function getLinks(): Promise<LinkItem[]> {
  return new Promise((resolve) => {
    chrome.storage.sync.get({ [LINK_KEY]: [] }, (r) => {
      const v = r[LINK_KEY];
      resolve(Array.isArray(v) ? (v as LinkItem[]) : []);
    });
  });
}
function saveLinks(v: LinkItem[]): Promise<void> {
...
const faviconFor = (u: string) =>
  `https://www.google.com/s2/favicons?domain=${encodeURIComponent(domainFrom(u))}&sz=64`;

// Links UI (guarded)
const linkUrl = document.getElementById("link-url") as HTMLInputElement | null;
const linkTitle = document.getElementById("link-title") as HTMLInputElement | null;
const addLinkBtn = document.getElementById("add-link-btn") as HTMLButtonElement | null;
const linksList = document.getElementById("links-list") as HTMLUListElement | null;

function renderLinks(items: LinkItem[]): void {
  if (!linksList) return;
  linksList.innerHTML = "";
  ...
  if (!items.length) {
    ...
    linksList.appendChild(empty as unknown as Node);
    return;
  }

  const sorted = [...items].sort(
    (a, b) => Number(b.favorite) - Number(a.favorite) || b.id - a.id
  );
  ...
  for (const it of sorted) {
    ...
    // Favorite toggle
    const favBtn = li.querySelector(".link-fav") as HTMLButtonElement;
    if (it.favorite) favBtn.classList.add("active");
    ...
    // Delete link
    const delBtn = li.querySelector(".link-del") as HTMLButtonElement;
...
// Tabs (guarded)
const tabNotes = document.getElementById("tab-notes") as HTMLButtonElement | null;
const tabLinks = document.getElementById("tab-links") as HTMLButtonElement | null;
const panelNotes = document.getElementById("panel-notes") as HTMLElement | null;
const panelLinks = document.getElementById("panel-links") as HTMLElement | null;

function switchTab(which: "notes" | "links") {
  if (!tabNotes || !tabLinks || !panelNotes || !panelLinks) return;
